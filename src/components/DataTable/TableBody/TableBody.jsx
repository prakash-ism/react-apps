      ...
      {data.map(row => {
        return (
          <tr
            key={row.id}
            className={
              checkedState[row.id - 1] ? 'table-row checked' : 'table-row'
            }
          >
            <td className="table-data">
              <label htmlFor={row.id} className="visuallyhidden">
            ...
            <td className="table-data">
              {editState[row.id - 1] ? (
                <span
                  onClick={() =>
                    handleOnSave(row.id, { id: row.id, name, email, role })
                  }
                >
                  <img src={saveLogo} alt="save icon" />
                </span>
              ) : (
                <>
                  <span onClick={() => handleOnEditClick(row.id - 1)}>
                    <img src={editLogo} alt="edit icon" />
                  </span>
                  <span onClick={() => deleteSingle(row.id)}>
